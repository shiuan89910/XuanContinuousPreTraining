...
def main(args):
    ...
    if args.json:
        ...
        for i in range(0, len(datasets)):
            try:
                ...
                loaded = load_dataset(datasets[i])[args.split] # Bug Fixed
            if len(splits[i]) > 0:
    ...
    tokenized_dataset = dataset.map(
        lambda example: {**tokenizer([t + tokenizer.eos_token for t in example[args.feature]]),
                         "labels": tokenizer([t + tokenizer.eos_token for t in example[args.feature]])["input_ids"]}, # Bug Fixed (Add Labels)
        batched=True,
